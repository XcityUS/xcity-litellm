...
def _sanitize_user_agent_config(request: Mapping[str, Any]) -> Any:
    ...
    litellm_params = {
        k: v for k, v in (request.get("litellm_params") or {}).items() if k in _USER_ALLOWED_LITELLM_PARAMS
    }
    if litellm_params:
...
async def _check_user_agent_quota(user_api_key_dict: UserAPIKeyAuth, prisma_client: Any) -> None:
    ...
    count = await AgentsRepository(prisma_client).table.count(where={"created_by": user_api_key_dict.user_id})
...
async def _resolve_visible_agent_ids(
    ...
    explicit_ids = set(await AgentRequestHandler.get_allowed_agents(user_api_key_auth=user_api_key_dict))
...
class _AgentHealthResult(TypedDict, total=False):
    agent_id: ReadOnly[Required[str]]
    healthy: ReadOnly[Required[bool]]
    error: ReadOnly[str]
    skipped: ReadOnly[bool]
...
async def _check_agent_url_health(
    ...
    ``error`` message.
    """
    card: Final = agent.agent_card_params or {}
    url: Final = card.get("url")
    if not url:
...
async def get_agents(
    ...
    q: str | None = Query(None, description="Free-text match against agent name + description."),
    ...
    cursor: str | None = Query(None, description="agent_id of the previous page tail for cursor pagination."),
    ...
    try:
        ...
        if (
            ...
            returned_agents = [agent for agent in all_agents if agent.agent_id in visible_ids]
        ...
        if category:
            returned_agents = [
                a for a in returned_agents if category in ((a.agent_card_params or {}).get("categories") or [])
            ]
        if tag:
            returned_agents = [a for a in returned_agents if tag in ((a.agent_card_params or {}).get("tags") or [])]
        if supports_streaming is not None:
            returned_agents = [
                a
                for a in returned_agents
                if bool(((a.agent_card_params or {}).get("capabilities") or {}).get("streaming")) == supports_streaming
            ]
...
async def list_agent_versions_endpoint(
    ...
    cursor: str | None = Query(None, description="version_id of the prev page tail."),
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
) -> list[dict[str, Any]]:
...
async def rollback_agent_endpoint(
    ...
    refreshed = await prisma_client.db.litellm_agentstable.find_unique(where={"agent_id": agent_id})
    if refreshed is None:
        raise HTTPException(status_code=404, detail=f"Agent {agent_id} vanished mid-rollback.")
    agent_dict = refreshed.model_dump() if hasattr(refreshed, "model_dump") else dict(refreshed)
