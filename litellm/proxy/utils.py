...
def create_model_info_response(
    ...
    Always includes a ``capabilities`` sub-object (S1-06) and, when known, a
    ``context_window`` / ``max_output_tokens`` pair so downstream apps can decide
    whether a model supports vision / tools / structured output without keeping
    their own provider matrix. When include_metadata is true, also attaches the
    model's configured fallbacks (resolved via the router under fallback_type,
    defaulting to "general"). Raises HTTPException(400) for an unknown fallback_type.
    """
    ...
    try:
        ...
        base["capabilities"] = flags.model_dump()
        if (context_window := coerce_token_limit(row.get("max_input_tokens") or row.get("max_tokens"))) is not None:
            base["context_window"] = context_window
        if (output_limit := coerce_token_limit(row.get("max_output_tokens"))) is not None:
            base["max_output_tokens"] = output_limit
