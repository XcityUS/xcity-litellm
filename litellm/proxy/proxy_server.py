...
class ProxyConfig:
    ...
    async def _run_startup_db_step(
        ...
        try:
            ...
            verbose_proxy_logger.info(f"[startup] db-load '{name}' ok in {time.time() - start:.1f}s")
        except asyncio.TimeoutError:
            verbose_proxy_logger.error(
                f"[startup] db-load '{name}' TIMED OUT after {timeout:.0f}s — skipping so startup can proceed"
            )
        except Exception as e:
            verbose_proxy_logger.warning(
                f"[startup] db-load '{name}' failed in {time.time() - start:.1f}s: {e} — skipping"
            )
    ...
    async def _init_non_llm_objects_in_db(self, prisma_client: PrismaClient):
        ...
        if self._should_load_db_object(object_type="vector_store_indexes"):
            await self._run_startup_db_step(
                ...
                lambda: self._init_vector_store_indexes_in_db(prisma_client=prisma_client),
            )

        if self._should_load_db_object(object_type="mcp"):
            await self._run_startup_db_step("mcp", lambda: self._init_mcp_servers_in_db())

        if self._should_load_db_object(object_type="agents"):
            await self._run_startup_db_step("agents", lambda: self._init_agents_in_db(prisma_client=prisma_client))
        ...
        if self._should_load_db_object(object_type="prompts"):
            await self._run_startup_db_step("prompts", lambda: self._init_prompts_in_db(prisma_client=prisma_client))
        ...
        if self._should_load_db_object(object_type="model_cost_map"):
            await self._run_startup_db_step(
                "model_cost_map",
                lambda: self._check_and_reload_model_cost_map(prisma_client=prisma_client),
            )
        ...
        if self._should_load_db_object(object_type="anthropic_beta_headers"):
            await self._run_startup_db_step(
                ...
                lambda: self._check_and_reload_anthropic_beta_headers(prisma_client=prisma_client),
            )
...
async def chat_completion(
    ...
    if data.get("skills"):
        ...
        data["metadata"]["skill_ids"] = [
            (s.split("@", 1)[0] if isinstance(s, str) else s) for s in data.get("skills", []) if isinstance(s, str)
        ]
