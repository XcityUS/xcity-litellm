...
def get_logging_payload(kwargs, response_obj, start_time, end_time) -> SpendLogsPayload:
    ...
    skill_ids: list[str] = [s for s in (metadata.get("skill_ids") or []) if isinstance(s, str)]
    app_id_val: str | None = metadata.get("user_api_key_app_id") or metadata.get("app_id") or kwargs.get("app_id")
    if agent_id:
        entity_type = "agent"
        entity_id = agent_id
        entity_version = None
