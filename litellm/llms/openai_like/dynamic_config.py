    ...
    class JSONProviderConfig(base_class):
        ...
        def get_supported_openai_params(self, model: str) -> list:
            ...
            filtered_params: Final = tuple(
                param for param in supported_params if not tools_disabled or param not in FUNCTION_CALLING_OPENAI_PARAMS
            )
            reasoning_params: Final = (
                ("reasoning_effort",)
                if supports_reasoning(model=model, custom_llm_provider=provider.slug)
                and "reasoning_effort" not in filtered_params
                else ()
            )
            return [*filtered_params, *reasoning_params]
