...
class TestAgentRBACProxyAdminViewOnly:
    ...
    def _setup(self, monkeypatch):
        ...
        self.allowed_agents_spy = AsyncMock(return_value=RestrictedAgentAccess(frozenset({"someone-elses-agent"})))
        monkeypatch.setattr(
...
class TestUserAgentWrites:
    ...
    def test_owner_may_modify_own_agent(self, monkeypatch):
        monkeypatch.setenv("AGENT_REGISTRY_ALLOW_USER_WRITES", "true")
        agent_endpoints._check_agent_management_permission(self._user_auth(), {"created_by": "creator-1"})
    ...
    def test_non_owner_blocked_from_others_agent(self, monkeypatch):
        ...
        monkeypatch.setenv("AGENT_REGISTRY_ALLOW_USER_WRITES", "true")
        with pytest.raises(HTTPException) as exc_info:
            agent_endpoints._check_agent_management_permission(self._user_auth(), {"created_by": "someone-else"})
        assert exc_info.value.status_code == 403
    ...
    def test_sanitize_partial_patch_keeps_absent_fields_absent(self):
        sanitized = agent_endpoints._sanitize_user_agent_config({"agent_card_params": {"name": "Renamed"}})
