...
it("preserves raw responses and request options through the injected transport", async () => {
  const response = new Response("stream payload", { status: 206 });
  const fetchImpl = vi.fn(async () => response);
  const client = createApiClient({ getBaseUrl: () => "https://proxy.example", fetchImpl });
  const body = new FormData();
  body.set("file", new Blob(["content"]));
  const signal = new AbortController().signal;
  const options: RequestInit = { method: "POST", headers: { "x-custom-auth": "test-token" }, body, signal };
  const result = await client.fetchResponse("https://upload.example/path", options);
  expect(result).toBe(response);
  expect(fetchImpl).toHaveBeenCalledWith("https://upload.example/path", options);
  expect(await result.text()).toBe("stream payload");
});
